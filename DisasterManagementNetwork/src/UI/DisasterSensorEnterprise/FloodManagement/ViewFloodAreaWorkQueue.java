...
public class ViewFloodAreaWorkQueue extends javax.swing.JPanel {
    ...
    public void populateTable(){
        ...
        FloodManagementWorkRequest floodreq = null;
            ...
            if(organization instanceof SensorMonitorOrganization)
            {
                for (WorkRequest workr : organization.getWorkQueue().getWorkRequestList()) {
                    if (workr instanceof FloodManagementWorkRequest) {
                        floodreq = (FloodManagementWorkRequest)workr;
            
                        for(FloodSensor fas : floodreq.getFloodManagementSensor())
                        {
                        ZipcodeList.add(fas.getZipcode());
                        }
                        Object[] row = new Object[7];
                        row[0] = floodreq;
                        row[1] = floodreq.getSender();                        
                        row[2] = convertListToCSV(ZipcodeList);
                        row[3] = floodreq.getStatus();
                        row[4] = floodreq.getMessage();
                        row[5] = floodreq.getRequestDate();
                        model.addRow(row);
                }
                }
            }
            ...
            if(organization instanceof SensorMonitorOrganization)
            {
                for (WorkRequest workr : organization.getWorkQueue().getWorkRequestList()) {
                    if (workr instanceof FloodManagementWorkRequest)  {
                        ...
                        if(!floodReq.getStatus().equalsIgnoreCase("Resolved"))
                        {
                            Object[] row = new Object[7];
                            row[0] = floodReq;
                            row[1] = floodReq.getSender();
//                            row[2] = floodReq.getFloodManagementSensor().getSensorId();
//                            row[3] = floodReq.getFloodManagementSensor().get;
                            row[2] = floodReq.getStatus();
                            row[3] = floodReq.getMessage();
                            row[4] = floodReq.getRequestDate();
                            model.addRow(row);
                        }
                    }
                }
...
//            Sensor airPollutionSenso = (Sensor)ResolvedWorkQueuejTable.getValueAt(row, 0);
            ...
            for(FloodSensor fas : reques.getFloodManagementSensor())
            {
            ZipcodeList.add(fas.getZipcode());
            }
